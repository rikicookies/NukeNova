...
Each article can be public, limited to registered members or limited to members with an active VIP entitlement. The selected audience is enforced in lists, categories, detail pages, comments and Search. RSS, sitemap entries and public profile article counts include public articles only.
...
News listens to `comments.content.checking`. It accepts only an existing published article whose `comments_enabled` flag is on and whose audience permits the current viewer. If the optional Comments module is disabled, News continues to render normally and shows no comment forms.

## RSS

News publishes RSS 2.0 at `/news/rss.xml`. It contains at most the 20 newest public articles whose publication time has arrived. Drafts, restricted articles, deleted articles and future scheduled articles are never included.
