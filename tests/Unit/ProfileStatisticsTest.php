...
{
 public function testProfileStatisticsAreProvidedOnlyByActiveModules():void{$root=dirname(__DIR__,2);$controller=file_get_contents($root.'/app/Auth/PublicProfileController.php');self::assertStringContainsString("dispatch('profile.statistics.building'",$controller);foreach(['News','Comments','Friends']as$module){$source=file_get_contents($root.'/modules/'.$module.'/src/'.$module.'Module.php');self::assertStringContainsString("listen('profile.statistics.building'",$source);}}
